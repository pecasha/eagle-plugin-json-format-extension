...
const useGraph = create<Graph & GraphActions>((set, get) => ({
    ...
    collapseGraph: () => {
        ...
        const secondDegreeNodesIds = edges.filter(edge => parentNodesIds.includes(edge.from)).map(edge => edge.to);

        const collapsedParents = get().nodes.filter(node => !parentNodesIds.includes(node.id) && node.data.isParent).map(node => node.id);

        const collapsedNodes = get().nodes.filter(
        ...
        set({
            collapsedParents,
            collapsedNodes,
            collapsedEdges: get().edges.filter(edge => !parentNodesIds.includes(edge.from)).map(edge => edge.id),
            graphCollapsed: true
        });
    },
    expandGraph: () => {
